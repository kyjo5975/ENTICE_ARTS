...
def scenarioplots(P,T,z,LWC,IWC,RR,SR,H2O,O3,n):
    ...
    fig, ax1 = plt.subplots()
    l = ax1.semilogx(LWC, z, c='b')
    ax2 = ax1.twiny()
    l2 = ax2.semilogx(IWC, z, c='c')
    ax1.set_xlabel('Liquid Water Content [log10(kg/m^3)]')
    ax2.set_xlabel('Ice Water Content [log10(kg/m^3)]')
    ax1.set_ylabel('Altitude [m]')
    plt.title(f"Scenario {n}")
    blue_line = mlines.Line2D([], [], color='b', label='LWC')
    cyan_line = mlines.Line2D([], [], color='c', label='IWC')
    plt.legend(handles=[blue_line, cyan_line])
    fig.tight_layout()

    fig, ax1 = plt.subplots()
    l = ax1.semilogx(RR, z, c='0.4')
    ax2 = ax1.twiny()
    l2 = ax2.semilogx(SR, z, c='0.8')
    ax1.set_xlabel('Rain Rate [log10(kg/(m2*s))]')
    ax2.set_xlabel('Snow Rate [log10(kg/(m2*s))]')
    ax1.set_ylabel('Altitude [m]')
    plt.title(f"Scenario {n}")
    grey1_line = mlines.Line2D([], [], color='0.4', label='RR')
    grey2_line = mlines.Line2D([], [], color='0.8', label='SR')
    plt.legend(handles=[grey1_line, grey2_line])
    fig.tight_layout()

    fig, ax1 = plt.subplots()
    l = ax1.semilogx(H2O, z, c='b')
    ax2 = ax1.twiny()
    l2 = ax2.semilogx(O3, z, c='g')
    ax1.set_xlabel('H2O [log10(%)]')
    ax2.set_xlabel('O3 [log10(%)]')
    ax1.set_ylabel('Altitude [m]')
    plt.title(f"Scenario {n}")
